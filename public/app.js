...
function openCallPopup(contact) {
  ...
  // Jeśli połączenie jest już zakończone — NIE pokazuj migającego wskaźnika
  const isAlreadyEnded = contact.status === 'ended' || contact.tag === 'connected' || contact.tag === 'missed' || contact.tag === 'ineffective';
  if (isAlreadyEnded) {
    if (pulseDot) pulseDot.style.display = 'none';
    if (statusText) {
      const statusLabels = { connected: '✅ Rozmowa zakończona', missed: '❌ Nieodebrane', ineffective: '⚫ Nieskuteczne' };
      const statusColors = { connected: '#10b981', missed: '#ef4444', ineffective: '#94a3b8' };
      statusText.textContent = statusLabels[contact.tag] || '✅ Połączenie zakończone';
      statusText.style.color = statusColors[contact.tag] || '#64748b';
      statusText.style.fontWeight = '700';
    }
  } else {
  ...
  try {
    ...
    if (activeCallId) {
      ...
      try {
        ...
        const currentCallNoteText = document.getElementById('currentCallNote')?.value?.trim() || '';
        ...
        const payload = {
          ...
          // Pierwsze / kolejne połączenie — ważne dla automatyzacji GHL
        ...
        // Zapisz bieżącą notatkę z rozmowy do GHL (jeśli uzupełniona)
        if (currentCallNoteText && contactId && contactId !== 'unknown') {
          try {
            ...
            console.log('[Report] Note saved to GHL:', currentCallNoteText.slice(0, 50));
...
function setCallType(type) {
  ...
  if (type === 'first') {
    ...
    nextBtn.style.borderColor = '#e2e8f0'; nextBtn.style.background = '#f8fafc'; nextBtn.style.color = '#64748b'; nextBtn.style.fontWeight = '400';
  } else {
    nextBtn.style.borderColor = '#3b82f6'; nextBtn.style.background = '#eff6ff'; nextBtn.style.color = '#1d4ed8'; nextBtn.style.fontWeight = '700';
    firstBtn.style.borderColor = '#e2e8f0'; firstBtn.style.background = '#f8fafc'; firstBtn.style.color = '#64748b'; firstBtn.style.fontWeight = '400';
  }
