...
// ==================== GHL STAGES + PATIENT TAGS (P5 — ładowane z API) ====================
// Lokalne kopie są inicjowane ze stałymi jako fallback.
// Przy starcie aplikacji nadpisujemy je danymi z /api/stages i /api/patient-status-tags.
// Dzięki temu zmiana stage UUID w GHL wymaga edycji tylko server.js.

let PATIENT_STATUS_TAGS = {
...
// Zbuduj GHL_STAGE_IDS z GHL_STAGE_NAMES (odwrotne mapowanie name→id)
let GHL_STAGE_IDS = Object.fromEntries(
  Object.entries(GHL_STAGE_NAMES).map(([id, name]) => [
    name.toUpperCase().replace(/[^A-Z0-9]/g, '_'), id
  ])
);

// Pobierz definicje z serwera i nadpisz lokalne kopie
async function loadServerDefinitions() {
  try {
    const [stagesRes, tagsRes] = await Promise.allSettled([
      fetch('/api/stages').then(r => r.json()),
      fetch('/api/patient-status-tags').then(r => r.json()),
    ]);
    if (stagesRes.status === 'fulfilled' && stagesRes.value?.stages) {
      GHL_STAGE_NAMES = stagesRes.value.stages;
      GHL_STAGE_IDS = Object.fromEntries(
        Object.entries(GHL_STAGE_NAMES).map(([id, name]) => [
          name.toUpperCase().replace(/[^A-Z0-9]/g, '_'), id
        ])
      );
    ...
    if (tagsRes.status === 'fulfilled' && tagsRes.value?.tags) {
      // Server zwraca obiekt {key, label, color, bg} — mapuj do formatu frontend
      const serverTags = tagsRes.value.tags;
      const mapped = {};
      Object.values(serverTags).forEach(t => {
        mapped[t.key] = { label: t.label, color: t.color, bg: t.bg, border: t.color + '55' };
      });
      if (Object.keys(mapped).length > 0) PATIENT_STATUS_TAGS = mapped;
    }
  } catch(e) {
    console.warn('[P5] Nie udało się załadować definicji z serwera, używam fallback:', e);
  }
}


// ─── apiFetch — wrapper fetch z X-API-Key (P2) ───────────────────────────────
// Klucz jest wstrzykiwany przez serwer jako meta tag w index.html:
//   <meta name="nav-api-key" content="...">
// Klucz jest odczytywany LENIWIE przy każdym wywołaniu (nie raz przy starcie),
// dzięki czemu działa poprawnie nawet gdy skrypt ładuje się przed pełnym DOM.
function _getNavApiKey() {
  const m = document.querySelector('meta[name="nav-api-key"]');
  return m ? m.getAttribute('content') : '';
}

async function apiFetch(url, options = {}) {
  const headers = { ...(options.headers || {}) };
  const key = _getNavApiKey();
  if (key) headers['X-API-Key'] = key;
  // Jeśli body jest obiektem — automatycznie dodaj Content-Type: application/json
  let body = options.body;
  if (body && typeof body === 'object' && !(body instanceof FormData)) {
    body = JSON.stringify(body);
    if (!headers['Content-Type']) headers['Content-Type'] = 'application/json';
  }
  return apiFetch(url, { ...options, headers, body });
}
...
function getPatientStatusBadge(tags) {
  ...
  for (const tag of tags) {
    ...
    if (def) return `<span style="font-size:10px;font-weight:600;padding:2px 7px;border-radius:10px;background:${def.bg};color:${def.color};border:1px solid ${def.border || def.color};white-space:nowrap;">${def.label}</span>`;
  }
  return '';
}
...
async function moveOpportunityToStage(oppId, stageId) {
  if (!oppId) return;
  try {
    await apiFetch(`/api/opportunity/${oppId}/move-stage`, {
...
function initApp() {
  ...
  if (currentUser?.id) {
    const sendHeartbeat = () => {
      apiFetch(`/api/users/${currentUser.id}/heartbeat`, { method: 'POST' }).catch(() => {});
    };
...
async function loadDashboardData() {
  ...
  try {
    ...
    const statsResp = await apiFetch(`/api/stats?days=1&userId=${uid}&role=${rol}`);
    if (statsResp.ok) {
  ...
  try {
    ...
    const callsResp = await apiFetch(`/api/calls?today=1&userId=${uid2}&role=${rol2}`);
    if (callsResp.ok) {
...
async function loadChatHistory(convKey) {
  ...
  if (!container) return;
  try {
    const r = await apiFetch(`/api/chat/history/${convKey}`);
    const data = await r.json();
...
async function loadSoniaInbox() {
  ...
  try {
    const r = await apiFetch('/api/chat/sonia-inbox');
    const data = await r.json();
...
async function loadSoniaChatHistory() {
  ...
  try {
    ...
    const r = await apiFetch(`/api/chat/history/${convKey}?limit=100`);
    const data = await r.json();
...
document.addEventListener('DOMContentLoaded', () => {
  // P5: załaduj definicje z serwera przed inicjalizacją logowania
  loadServerDefinitions().finally(() => initLoginScreen());
});
...
async function loadNewLeads() {
  ...
  try {
    const response = await apiFetch('/api/opportunities/new');
...
async function deleteLead(oppId, btn) {
  ...
  btn.disabled = true;
  try {
    const r = await apiFetch(`/api/opportunity/${oppId}`, { method: 'DELETE' });
    if (r.ok) {
...
function renderCallRow(c, isAdmin = false) {
  ...
  const contactTypeMap = {
    'NOWY_PACJENT':    { label: 'Nowy pacjent',    cls: 'ct-new', letter: 'N' },
    'STALY_PACJENT':   { label: 'Stały pacjent',   cls: 'ct-regular', letter: 'S' },
    'WIZYTA_BIEZACA':  { label: 'Bieżąca wizyta',  cls: 'ct-visit', letter: 'W' },
    'SPAM':            { label: 'Pomyłka/SPAM',    cls: 'ct-spam', letter: 'X' }
  };
  ...
  let statusIconHtml = '';
  if (isEnded && !hasReport) {
    statusIconHtml = '<div class="status-icon-dot si-missing" title="Brak raportu">!</div>';
  } else if (ctInfo) {
    statusIconHtml = `<div class="status-icon-dot si-${c.contactType.toLowerCase()}" title="${ctInfo.label}">${ctInfo.letter}</div>`;
  } else {
    statusIconHtml = '<div class="status-icon-dot si-none"></div>';
  }

  // Ogólny status pacjenta (kropka + label)
  let patientStatusHtml = '';
  if (c.patientStatus) {
    const ps = c.patientStatus;
    patientStatusHtml = `<div style="display:flex;align-items:center;gap:4px;margin-top:2px;">
      <span class="status-dot-mini sd-${ps}"></span>
      <span style="font-size:10px;color:#64748b;font-weight:600;text-transform:uppercase;">${ps.replace(/_/g, ' ')}</span>
    </div>`;
  }
  ...
  const fullName = escHtml(c.contactName || (c.from !== '0' ? c.from : '') || (c.to !== '0' ? c.to : '') || 'Nieznany');
    ...
    <tr class="call-row" onclick="openCallReport('${c.callId}')">
      <td>
        <div style="display:flex;align-items:center;gap:10px;">
          ${statusIconHtml}
          <div class="call-avatar ${c.direction === 'outbound' ? 'av-out' : 'av-in'}">${fullName.charAt(0).toUpperCase()}</div>
          <div class="call-name-cell">
            <div class="call-name" onclick="event.stopPropagation();openPatientCard('${c.contactId}', '${fullName}')" style="cursor:pointer;color:#3b82f6;text-decoration:underline;">${fullName}</div>
            <div class="call-number" onclick="event.stopPropagation();openPatientCard('${c.contactId}', '${fullName}')" style="cursor:pointer;">${escHtml((c.from !== '0' ? c.from : '') || (c.to !== '0' ? c.to : '') || '')}</div>
            ${patientStatusHtml}
      ...
      <td onclick="event.stopPropagation()">
        <div style="display:flex;flex-direction:column;gap:4px;">
          ${(c.from || c.to) ? `<button class="btn-call btn-sm" onclick="initiateCall('${c.from || c.to}', '${fullName}')">📞 Oddzwoń</button>` : ''}
          ${noteHtml}
        </div>
      </td>
    </tr>
...
async function fetchRecording(callId, btn) {
  ...
  btn.textContent = '⏳';
  try {
    const r = await apiFetch(`/api/call/${callId}/recording`);
    const data = await r.json();
    if (data.url) {
...
async function openCallReport(callId) {
  ...
  try {
    const r = await apiFetch(`/api/calls/${callId}/report`);
    if (r.ok) {
...
function openCallPopup(contact) {
  ...
  // Ogólny status pacjenta w popup
  const popupStatusBadge = document.getElementById('popupPatientStatusBadge');
  if (popupStatusBadge) {
    if (contact.patientStatus) {
      const ps = contact.patientStatus;
      const def = PATIENT_STATUS_TAGS[ps] || { label: ps, bg: '#f1f5f9', color: '#475569', border: '#e2e8f0' };
      popupStatusBadge.innerHTML = `<span class="patient-status-badge ps-${ps}" style="background:${def.bg}; color:${def.color}; border:1px solid ${def.border};">${def.label}</span>`;
    } else {
      popupStatusBadge.innerHTML = '';
    }
  }
...
async function fetchPopupEnrichment(contactId) {
  try {
    const response = await apiFetch(`/api/contact/${contactId}/popup`);
    if (!response.ok) return;
    const data = await response.json();

    // Automatyczne zaciąganie źródła leada
    const zrodloInput = document.getElementById('zrodloLeada');
    if (zrodloInput && !zrodloInput.value && data.source) {
      zrodloInput.value = data.source;
    }
...
async function hangupCall() {
  ...
  if (activeCallId) {
    try {
      const resp = await apiFetch('/api/call/hangup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function saveReport(forceClose = true) {
  ...
  try {
    ...
    if (activeCallId) {
      ...
      crmPromises.push(apiFetch(`/api/calls/${activeCallId}/report`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (currentCallNoteText && contactId && contactId !== 'unknown') {
        crmPromises.push(apiFetch(`/api/contact/${contactId}/notes`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
async function handleNewPatientReport(contactId, data) {
  ...
  if (contactId && contactId !== 'unknown') {
    ...
    if (customFieldsToUpdate.length > 0) {
      try {
        await apiFetch(`/api/contact/${contactId}`, {
  ...
  if (data.outcome === 'umowil_sie' && data.dataW0) {
    if (currentOpportunity?.id) {
      await apiFetch(`/api/opportunity/${currentOpportunity.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' },
    ...
    if (contactId && contactId !== 'unknown') {
      ...
      apiFetch(`/api/contact/${contactId}/task`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    if (contactId && contactId !== 'unknown' && (powod || notatka)) {
      ...
      await apiFetch(`/api/contact/${contactId}/notes`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function handleVisitReport(contactId, data) {
  if (data.outcome === 'zmiana_terminu') {
    ...
    if (contactId && contactId !== 'unknown') {
      await apiFetch(`/api/contact/${contactId}/task`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function loadTodayTasks() {
  ...
  try {
    ...
    const r = await apiFetch(`/api/tasks?userId=${userId}&filter=mine`);
    const data = await r.json();
...
async function loadTasks() {
  try {
    ...
    const [myResp, allResp, poolResp] = await Promise.all([
      apiFetch(`/api/tasks?userId=${userId}&filter=mine`),
      apiFetch(`/api/tasks?filter=all`),
      apiFetch(`/api/tasks?filter=unassigned`)
    ]);
    ...
    const activityResp = await apiFetch('/api/users/activity').catch(() => null);
    if (activityResp?.ok) {
...
async function loadDashboardPool() {
  ...
  try {
    const r = await apiFetch('/api/tasks?filter=unassigned');
    const data = await r.json();
...
async function loadContacts() {
  ...
  try {
    const response = await apiFetch('/api/contacts/new');
    const data = await response.json();
...
function openPatientCardFromPopup() {
  if (currentContact && currentContact.id) {
    openPatientCard(currentContact.id, currentContact.name);
  }
}

function openCreateTaskModal() {
  // Otwiera widok zadań i odpala modal (jeśli jest taka funkcja)
  // W tej wersji po prostu przełączamy na widok zadań
  switchView('tasks');
  showToast('Przełączono na widok zadań. Możesz teraz dodać nowe zadanie.', 'info');
}

async function markAsUnqualified() {
  if (!currentContact || !currentContact.id) return;
  if (!confirm('Czy na pewno oznaczyć kontakt jako niekwalifikowany?')) return;
  
  try {
    await apiFetch(`/api/contact/${currentContact.id}/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'niekwalifikowany' })
    });
    showToast('✅ Oznaczono jako niekwalifikowany', 'success');
    document.getElementById('callPopup').classList.add('hidden');
    loadDashboardData();
...
function markAsResigned() {
  document.getElementById('resignationModal').style.display = 'flex';
  document.getElementById('resignationReason').focus();
}
...
async function confirmResignation() {
  ...
  if (!reason) { alert('Proszę podać powód rezygnacji'); return; }
  
  try {
    await apiFetch(`/api/contact/${currentContact.id}/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'rezygnacja', reason })
    });
    showToast('✅ Rezygnacja zapisana', 'success');
    closeResignationModal();
    document.getElementById('callPopup').classList.add('hidden');
    loadDashboardData();
  } catch(e) { showToast('Błąd zapisu rezygnacji', 'error'); }
}
...
async function submitEditRequest() {
  ...
  try {
    const response = await apiFetch(`/api/contact/${editRequestContactId}/request-edit`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function loadAndRenderStats() {
  try {
    ...
    const r = await apiFetch(`/api/stats?days=${days}&userId=${uid3}&role=${rol3}`);
    ...
    if (currentUser?.role === 'admin') {
      try {
        const rr = await apiFetch(`/api/reports/stats?days=${days}`);
        if (rr.ok) {
...
function renderCharts(statsData) {
  ...
  if (!statsData) {
    { const uid4 = currentUser?.id || ''; const rol4 = currentUser?.role || 'reception'; apiFetch(`/api/stats?userId=${uid4}&role=${rol4}`).then(r => r.json()).then(d => renderCharts(d)).catch(() => renderCharts({})); }
    return;
  }
...
async function openDiagnose() {
  ...
  try {
    const r = await apiFetch('/api/call/diagnose');
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
...
async function openPatientCard(contactId, contactName) {
  ...
  try {
    const response = await apiFetch(`/api/contact/${contactId}/card`);
    if (!response.ok) {
...
function renderPatientCardInfo(data) {
  ...
  // Ogólny status pacjenta w karcie
  const psBadge = document.getElementById('patientCardStatusBadge');
  if (psBadge) {
    // Sprawdź status w tagach jeśli nie ma w patientStatus
    const tags = contact.tags || [];
    let ps = contact.patientStatus;
    if (!ps) {
        for (const t of tags) {
            if (PATIENT_STATUS_TAGS[t]) { ps = t; break; }
        }
    }
    
    if (ps) {
      const def = PATIENT_STATUS_TAGS[ps] || { label: ps, bg: '#f1f5f9', color: '#475569', border: '#e2e8f0' };
      psBadge.innerHTML = `<span class="patient-status-badge ps-${ps}" style="background:${def.bg}; color:${def.color}; border:1px solid ${def.border}; font-size:12px; font-weight:700; padding:4px 12px; border-radius:12px;">${def.label}</span>`;
    } else {
      psBadge.innerHTML = '';
    }
  }
  ...
  // Tagi kontaktu
  const tagsEl = document.getElementById('patientCardTags');
  if (tagsEl) {
    const tags = contact.tags || [];
    if (tags.length > 0) {
      tagsEl.innerHTML = tags.map(t => `<span class="patient-tag" style="display:inline-block; background:#f1f5f9; color:#475569; padding:2px 8px; border-radius:4px; font-size:11px; margin:2px;">${escHtml(t)}</span>`).join('');
    } else {
      tagsEl.innerHTML = '<span style="color:#94a3b8; font-size:12px; font-style:italic;">Brak tagów</span>';
    }
  }
...
async function loadPatientNotes() {
  ...
  try {
    const resp = await apiFetch(`/api/contact/${contactId}/notes`);
...
async function deletePatientNote(contactId, noteId) {
  ...
  try {
    await apiFetch(`/api/contact/${contactId}/notes/${noteId}`, { method: 'DELETE' });
...
async function loadCalls() {
  ...
  try {
    ...
    const response = await apiFetch(`/api/calls/history?${params}`);
    const data = await response.json();
    allCalls = data.calls || [];
    renderCallsTable(allCalls);
  } catch (err) {
...
function startHeartbeat() {
  ...
  setInterval(async () => {
    try {
      await apiFetch(`/api/users/${currentUser.id}/heartbeat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
