...
function openCallPopup(contact) {
  ...
  // Reset statusu — przywróć migającą kropkę i tekst kierunku
  const statusText = document.getElementById('popupStatusText');
  if (statusText) {
  ...
  if (fnInput && !fnInput.value && contact.name && contact.name !== 'Nieznany') {
...
function resetReportForm() {
  ...
  ['programLeczenia', 'dataW0', 'contactDateTime', 'powodRezygnacji',
   ...
   'manualFirstName', 'manualLastName', 'notatkaZPierwszejRozmowy', 'zrodloLeada', 'zrodloKontaktu'].forEach(id => {
    const el = document.getElementById(id);
  ...
  try {
    ...
    if (activeCallId) {
      ...
      try {
        ...
        // Wyślij notatkę bieżącą do GHL (jeśli wypełniona)
        if (contactId && contactId !== 'unknown' && reportData.callNoteGHL) {
          try {
            await fetch(`/api/contact/${contactId}/notes`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ body: reportData.callNoteGHL, title: `Notatka z rozmowy ${new Date().toLocaleDateString('pl-PL')}` })
            });
            console.log('[Report] Call note sent to GHL');
          } catch(e) { console.warn('[Report] GHL note error:', e.message); }
        }
...
function buildReportData() {
  ...
  const data = { 
    ...
    callNoteGHL: document.getElementById('callNoteGHL_staly')?.value || document.getElementById('callNoteGHL_wizyta')?.value || ''
  };
...
async function openPatientCard(contactId, contactName) {
  ...
  // Zapisz contactId karty do globalnej zmiennej (potrzebne do dodawania notatek)
  window.patientCardContactId = contactId;
...
function renderPatientCardInfo(data) {
  ...
  if (pipeline) {
    setField('patientCardPipeline', pipeline.pipelineName || pipeline.id);
...
async function renderPatientCardTimeline() {
  const contactId = window.patientCardContactId;
  const timelineEl = document.getElementById('patientCardTimeline');
  if (!timelineEl) return;
  if (!contactId || contactId === 'unknown') {
    timelineEl.innerHTML = '<div style="padding:16px;text-align:center;color:#94a3b8;">Brak powiązanego kontaktu GHL</div>';
    return;
  }
  timelineEl.innerHTML = '<div style="padding:16px;text-align:center;color:#94a3b8;">⏳ Ładowanie notatek z GHL...</div>';
  try {
    ...
    let html = `
      <div style="padding:12px;background:#f1f5f9;border-radius:8px;margin-bottom:16px;border:1px solid #e2e8f0;">
        <div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:8px;">➕ Dodaj nową notatkę do GHL</div>
        <textarea id="newNoteBody" class="form-textarea" rows="2" placeholder="Wpisz treść notatki..." style="margin-bottom:8px;font-size:13px;"></textarea>
        <button onclick="submitNewNote()" class="btn-primary" style="width:100%;padding:6px;font-size:12px;">💾 Zapisz w GHL</button>
      </div>
    `;

    if (notes.length === 0) {
      html += '<div style="padding:16px;text-align:center;color:#94a3b8;">Brak notatek w GHL dla tego kontaktu</div>';
    } else {
      notes.sort((a, b) => new Date(b.dateAdded || b.dateUpdated || 0) - new Date(a.dateAdded || a.dateUpdated || 0));
      html += notes.map(note => {
        const dateStr = note.dateAdded ? new Date(note.dateAdded).toLocaleString('pl-PL') : '';
        const titleHtml = note.title ? `<div style="font-size:11px;font-weight:700;color:#374151;margin-bottom:4px;">${escHtml(note.title)}</div>` : '';
        return `<div style="padding:12px;border-left:3px solid #8b5cf6;background:#f8fafc;border-radius:6px;margin-bottom:8px;position:relative;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <span style="font-size:10px;padding:2px 8px;border-radius:12px;background:#ede9fe;color:#7c3aed;font-weight:700;">Notatka GHL</span>
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:11px;color:#94a3b8;">${dateStr}</span>
              <button onclick="deleteGHLNote('${contactId}','${note.id}',this)" title="Usuń notatkę" style="background:none;border:none;cursor:pointer;color:#ef4444;font-size:14px;padding:0 2px;line-height:1;">&#x1F5D1;</button>
            </div>
          </div>
          ${titleHtml}
          <div style="font-size:13px;color:#1e293b;white-space:pre-wrap;">${escHtml(note.body || note.text || '—')}</div>
        </div>`;
      }).join('');
    }
    timelineEl.innerHTML = html;
  } catch (err) {
    timelineEl.innerHTML = `<div style="padding:16px;background:#fef2f2;border-radius:8px;color:#ef4444;">Błąd pobierania notatek: ${escHtml(err.message)}</div>`;
  }
...
async function submitNewNote() {
  const contactId = window.patientCardContactId;
  const bodyEl = document.getElementById('newNoteBody');
  const body = bodyEl?.value?.trim();
  if (!body) { showToast('Wpisz treść notatki', 'error'); return; }
  if (!contactId || contactId === 'unknown') { showToast('Brak powiązanego kontaktu GHL', 'error'); return; }
  try {
    const resp = await fetch(`/api/contact/${contactId}/notes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ body, title: \`Notatka \${new Date().toLocaleDateString('pl-PL')}\` })
    });
    if (!resp.ok) throw new Error(\`HTTP \${resp.status}\`);
    if (bodyEl) bodyEl.value = '';
    showToast('✅ Notatka zapisana w GHL', 'success');
    window._patientCardRendered.timeline = false; // Wymuś re-render
    await renderPatientCardTimeline();
  } catch (err) {
    showToast(\`Błąd zapisu notatki: \${err.message}\`, 'error');
  }
}

async function deleteGHLNote(contactId, noteId, btn) {
  if (!confirm('Czy na pewno usunąć tę notatkę z GHL?')) return;
  try {
    btn.disabled = true;
    const resp = await fetch(`/api/contact/${contactId}/notes/${noteId}`, { method: 'DELETE' });
    if (!resp.ok) throw new Error(\`HTTP \${resp.status}\`);
...
function closePatientCard() {
  ...
  window.patientCardContactId = null;
}
