    ...
    const ghlPromises = [
      ...
      axios.get(
       ...
       .catch(e => {
         ...
         if (s === 401) throw new Error('GHL token wygasł — odśwież token w .env');
         if (s === 404) throw new Error('Kontakt nie istnieje w GHL');
         throw new Error(`GHL niedostępny (HTTP ${s})`);
       }),
