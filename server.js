...
// CORS — obsługuje wiele domen (ALLOWED_ORIGINS jako lista rozdzielona przecinkami)
const allowedOrigins = (process.env.ALLOWED_ORIGINS || process.env.ALLOWED_ORIGIN || 'http://localhost:3000')
  .split(',').map(o => o.trim()).filter(Boolean);
app.use(cors({
  origin: (origin, cb) => {
    // Brak origin (curl, Postman, serwer→serwer) — przepuść
...
// ─── API KEY MIDDLEWARE (P2 — RODO) ─────────────────────────────────────────
// Ustaw NAVIGATOR_API_KEY w .env. Frontend wysyła go jako nagłówek X-API-Key.
// Webhooks Zadarma i GHL są wykluczone (nie mogą wysyłać nagłówka).
// Endpointy publiczne (/) i WebSocket też są wykluczone.
const NAVIGATOR_API_KEY = process.env.NAVIGATOR_API_KEY;
if (!NAVIGATOR_API_KEY) {
  console.warn('[Auth] UWAGA: NAVIGATOR_API_KEY nie jest ustawiony w .env — API jest OTWARTE!');
}
const API_KEY_BYPASS_PATHS = [
  '/webhook/zadarma',
  '/webhook/ghl',
  '/api/webhook/appointments',
  '/api/health',
  '/api/stages',             // ładowane przed logowaniem (loadServerDefinitions)
  '/api/patient-status-tags', // ładowane przed logowaniem (loadServerDefinitions)
];
app.use((req, res, next) => {
  // Tylko ścieżki /api/* wymagają klucza (statyczne pliki i SPA — wolne)
  if (!req.path.startsWith('/api/')) return next();
  // Bypass dla whitelisty
  if (API_KEY_BYPASS_PATHS.some(p => req.path.startsWith(p))) return next();
  // Jeśli klucz nie jest skonfigurowany — przepuść (dev mode), ale ostrzeż
  if (!NAVIGATOR_API_KEY) return next();
  const provided = req.headers['x-api-key'];
  if (provided === NAVIGATOR_API_KEY) return next();
  console.warn(`[Auth] Odrzucono żądanie do ${req.path} — brak lub błędny X-API-Key`);
  return res.status(401).json({ error: 'Unauthorized — brak X-API-Key' });
});
...
// ─── GHL CUSTOM FIELD IDs (single source of truth — P3) ──────────────────────
// Zmień tylko tutaj gdy GHL zwróci inne ID po przebudowie formularza.
const GHL_FIELD = {
  MAIN_PROBLEM:      'k1OizGtL0V6IaWjGlVBK', // z_czym_si_zgasza
  MARKETING_CONSENT: 'R0X7n8GG7545mnrGnREg', // zgoda_marketingowa
  W0_DATE:           'IUjxWY10y6kuITsSjfSw', // data_wizyty_w0
  W0_NOTES:          'v04mALNDZzMgyH8YzK46', // notatki_w0
};
...
app.get('/api/opportunities/new', async (req, res) => {
  ...
  try {
    ...
    // GHL_FIELD stałe są zdefiniowane globalnie (patrz: P3)
    ...
    const enriched = await batchEnrich(opportunities, async (opp) => {
      try {
        ...
        opp.contact = {
          ...
          tags:      contact.tags      || [],
          z_czym_si_zgasza:  getField(GHL_FIELD.MAIN_PROBLEM)?.value || '',
          marketing_consent: getField(GHL_FIELD.MARKETING_CONSENT)?.value === 'tak' || getField(GHL_FIELD.MARKETING_CONSENT)?.value === true,
          w0_date:    getField(GHL_FIELD.W0_DATE)?.value ? new Date(Number(getField(GHL_FIELD.W0_DATE).value)).toISOString() : null,
          w0_notes:   getField(GHL_FIELD.W0_NOTES)?.value || '',
...
app.get('/api/contacts/new', async (req, res) => {
  ...
  try {
    ...
    contacts.forEach(c => {
      ...
      const getField = (id) => cf.find(f => f.id === id);
      const mainProblem = getField(GHL_FIELD.MAIN_PROBLEM);
      const marketing   = getField(GHL_FIELD.MARKETING_CONSENT);
      const w0Date      = getField(GHL_FIELD.W0_DATE);
      const w0Notes     = getField(GHL_FIELD.W0_NOTES);
...
// Aktualizacja ogólnego statusu pacjenta (Niekwalifikacja/Rezygnacja/itp)
app.post('/api/contact/:id/status', async (req, res) => {
  try {
    const { id } = req.params;
    const { status, reason } = req.body;
    
    // 1. Ustaw tag w GHL
    const tagKey = status === 'rezygnacja' ? PATIENT_STATUS_TAGS.REZYGNACJA.key : PATIENT_STATUS_TAGS.NIEKWALIFIKOWANY.key;
    await setPatientStatusTag(id, tagKey);
    
    // 2. Jeśli jest powód rezygnacji, dodaj notatkę do GHL
    if (status === 'rezygnacja' && reason) {
      await axios.post(
        `https://services.leadconnectorhq.com/contacts/${id}/notes`,
        { body: `Powód rezygnacji: ${reason}`, userId: GHL_SONIA_USER_ID },
        { headers: ghlHeaders, timeout: 8000 }
      ).catch(() => {});
    }
    
    // 3. Zaktualizuj w Supabase
    if (supabase) {
      await supabase.from('contacts').upsert({
        ghl_contact_id: id,
        contact_status: status,
        updated_at: new Date().toISOString()
      }, { onConflict: 'ghl_contact_id' }).catch(() => {});
    }
  ...
  if (event === 'NOTIFY_START' || event === 'INCOMING') {
    ...
    if (existing) {
      ...
      storeCall({
        ...
        from: patientPhone,   // numer pacjenta (do kogo dzwonimy) — wyświetlany w UI
        to: patientPhone,
        agentPhone: caller,   // numer komórki agenta — tylko do logów, nie do UI
...
// P4: używa SimpleCache zamiast global.popupCache
const popupCache = new SimpleCache();
...
app.get('/api/contact/:id/popup', async (req, res) => {
  ...
  const cached = popupCache.get(contactId);
  if (cached) {
  ...
  try {
    ...
    const getField = (id) => cf.find(f => f.id === id);
    const mainProblem = getField(GHL_FIELD.MAIN_PROBLEM);
    const w0DateField = getField(GHL_FIELD.W0_DATE);
    ...
    popupCache.set(contactId, popupData, 30 * 1000); // 30s TTL
    res.json(popupData);
  } catch (err) {
...
app.get('/api/contact/:id/card', async (req, res) => {
  try {
    ...
    // Pipeline name cache — używa SimpleCache (TTL 24h)
    const KNOWN_PIPELINE_ID = 'FVgB3ga52b0PUi6QjJ0x';
    let pipelineName = cache.get(`pipeline_name_${KNOWN_PIPELINE_ID}`) || null;
    ...
    const ghlPromises = [
      ...
      ...(pipelineName ? [] : [
        axios.get(
          ...
          if (name) {
            cache.set(`pipeline_name_${KNOWN_PIPELINE_ID}`, name, 24 * 60 * 60 * 1000);
            pipelineName = name;
          } else {
            // Fallback: weź pierwszy pipeline jeśli tylko jeden
            pipelineName = pipelines[0]?.name || 'Navigator';
            cache.set(`pipeline_name_${KNOWN_PIPELINE_ID}`, pipelineName, 24 * 60 * 60 * 1000);
          }
        }).catch(() => {
          pipelineName = cache.get(`pipeline_name_${KNOWN_PIPELINE_ID}`) || 'Navigator';
        })
      ])
    ];
    ...
    const mainProblem    = getField(GHL_FIELD.MAIN_PROBLEM) || getFieldByKey('z_czym_si_zgasza');
    const marketing      = getField(GHL_FIELD.MARKETING_CONSENT) || getFieldByKey('zgoda_marketingowa');
    ...
    const w0DateField    = getField(GHL_FIELD.W0_DATE);
    const w0NotesField   = getField(GHL_FIELD.W0_NOTES);
    ...
    cache.set(cacheKey, responseData, 60 * 1000); // 60s TTL
    ...
    try {
      ...
      for (const row of filteredData) {
        ...
        uniqueCalls.push({
          ...
          // call_tag = status połączenia (connected/missed/ineffective)
          // contact_type = typ raportu (NOWY_PACJENT etc) — NIE używaj jako tag
          tag: row.call_tag || (row.status === 'ended' && row.duration_seconds > 0 ? 'connected' : row.direction === 'inbound' ? 'missed' : 'ineffective'),
          contactType: row.contact_type,
          hasReport: !!(row.report_saved_at), // true tylko gdy raport faktycznie zapisano
          callEffect: row.call_effect,
          notes: row.notes,
          program: row.treatment,
          outcome: row.call_reason,
...
// ─── Fallback SPA — wstrzykuje API key do meta tagu (P2) ──────────────────────
const indexHtmlPath = path.join(__dirname, 'public', 'index.html');
let _indexHtmlCache = null;
// Załaduj index.html do pamięci przy starcie (nie czytaj z dysku przy każdym żądaniu)
try { _indexHtmlCache = fs.readFileSync(indexHtmlPath, 'utf8'); } catch(e) {}

app.get('*', (req, res) => {
  if (!_indexHtmlCache) return res.sendFile(indexHtmlPath);
  const key = NAVIGATOR_API_KEY || '';
  const html = _indexHtmlCache.replace('__NAV_API_KEY__', key);
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  // Nie cache'uj HTML — klucz może się zmienić po restarcie
...
// Używa RENDER_EXTERNAL_URL lub ALLOWED_ORIGIN jako adres bazowy
// UWAGA: usuń jeśli masz płatny plan Render — niepotrzebne
const _renderBaseUrl = process.env.RENDER_EXTERNAL_URL
  || process.env.ALLOWED_ORIGINS?.split(',')[0]?.trim()
  || process.env.ALLOWED_ORIGIN?.trim();
if (_renderBaseUrl) {
  const keepAliveUrl = `${_renderBaseUrl}/api/health`;
  setInterval(async () => {
