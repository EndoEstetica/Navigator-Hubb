...
app.get('/api/contact/:id/card', async (req, res) => {
  try {
    ...
    const ghlPromises = [
      ...
      ...(pipelineName ? [] : [
        axios.get(
          `https://services.leadconnectorhq.com/opportunities/pipelines/${KNOWN_PIPELINE_ID}?locationId=${GHL_LOCATION_ID}`,
          { headers: ghlHeaders, timeout: 4000 }
        ).then(r => {
          const name = r.data?.pipeline?.name || r.data?.name || 'Navigator';
