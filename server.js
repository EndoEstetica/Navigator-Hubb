...
const GHL_PIPELINE_ID  = process.env.GHL_PIPELINE_ID || 'FVgB3ga52b0PUi6QjJ0x';
// Mapa nazw pipelineów (ID → nazwa) — Pipeline ID: FVgB3ga52b0PUi6QjJ0x
const GHL_PIPELINE_NAMES = {
  'FVgB3ga52b0PUi6QjJ0x': 'EndoEstetica'
};
...
const MAX_CONCURRENT_ZADARMA = 5; // max 5 jednoczesnych requestów do Zadarma (zwiększone — fix wydajności)
...
// Cache in-memory (60s TTL) — zapobiega wielokrotnym żądaniom do GHL przy otwieraniu popupu
if (!global.contactDetailCache) global.contactDetailCache = new Map();
setInterval(() => {
  const now = Date.now();
  for (const [k, v] of global.contactDetailCache.entries()) {
    if (now - v.ts > 60000) global.contactDetailCache.delete(k);
  }
}, 120000);
// Szczegóły kontaktu
app.get('/api/contact/:id', async (req, res) => {
  const contactId = req.params.id;
  if (!contactId || contactId === 'unknown' || contactId.length < 5) {
    return res.json({ contact: {}, id: contactId });
  }
  // Sprawdź cache
  const cached = global.contactDetailCache.get(contactId);
  ...
  try {
    ...
    // Zapisz do cache
    global.contactDetailCache.set(contactId, { ts: Date.now(), data: response.data });
...
// Cache dla /api/contact/:id/popup (30s)
...
// ─── NOTATKI GHL (dwukierunkowa synchronizacja) ──────────────────────────────
app.get('/api/contact/:id/notes', async (req, res) => {
  const contactId = req.params.id;
  if (!contactId || contactId === 'unknown') return res.json({ notes: [] });
  try {
    const response = await axios.get(
    ...
    res.status(500).json({ error: err.message, notes: [] });
  }
});

app.post('/api/contact/:id/notes', async (req, res) => {
  const contactId = req.params.id;
  if (!contactId || contactId === 'unknown') return res.status(400).json({ error: 'Nieprawidłowy contactId' });
  const { body, title, userId } = req.body;
  if (!body) return res.status(400).json({ error: 'Treść notatki jest wymagana' });
  try {
    const response = await axios.post(
      `https://services.leadconnectorhq.com/contacts/${contactId}/notes`,
      { body, title: title || 'Notatka', pinned: false },
      { headers: ghlHeaders, timeout: 10000 }
    );
    if (global.patientCardCache) global.patientCardCache.delete(`card_${contactId}`);
    res.json(response.data);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.delete('/api/contact/:id/notes/:noteId', async (req, res) => {
  const { id: contactId, noteId } = req.params;
  try {
    await axios.delete(
      `https://services.leadconnectorhq.com/contacts/${contactId}/notes/${noteId}`,
      { headers: ghlHeaders, timeout: 8000 }
    );
    if (global.patientCardCache) global.patientCardCache.delete(`card_${contactId}`);
...
app.post('/webhook/zadarma', async (req, res) => {
  ...
  if (event === 'NOTIFY_START' || event === 'INCOMING') {
    ...
    // Async: szukaj kontaktu w GHL po numerze telefonu (nie blokuj webhooka)
    if (GHL_TOKEN && caller) {
      lookupGHLContact(caller).then(contact => {
        if (contact) {
          storeCall({ callId, contactName: `${contact.firstName || ''} ${contact.lastName || ''}`.trim(), contactId: contact.id });
          broadcast({ type: 'CALL_ENRICHED', callId, contactName: `${contact.firstName || ''} ${contact.lastName || ''}`.trim(), contactId: contact.id, firstName: contact.firstName, lastName: contact.lastName });
        }
      }).catch(() => {});
    }
  }
...
app.get('/api/contact/:id/popup', async (req, res) => {
  const contactId = req.params.id;
  if (!contactId || contactId === 'unknown' || contactId.length < 5) {
    return res.json({ id: contactId, contact: {}, pipeline: null });
  }
  ...
  try {
    ...
    const [contactResp, oppsResp] = await Promise.allSettled([
      axios.get(`https://services.leadconnectorhq.com/contacts/${contactId}`, { headers: ghlHeaders, timeout: 6000 }),
      axios.get(`https://services.leadconnectorhq.com/opportunities/search?location_id=${GHL_LOCATION_ID}&contact_id=${contactId}&limit=1`, { headers: ghlHeaders, timeout: 6000 })
    ]);
    ...
    const pipelineName = latestOpp ? (GHL_PIPELINE_NAMES[latestOpp.pipelineId] || latestOpp.pipelineId || null) : null;
    const popupData = {
      id: contactId,
  ...
  try {
    ...
    const responseData = {
      ...
      pipeline: firstOpp ? {
        ...
        pipelineName: GHL_PIPELINE_NAMES[firstOpp.pipelineId] || firstOpp.pipelineId || 'Nieznany',
        stageName: GHL_STAGES[firstOpp.pipelineStageId] || firstOpp.pipelineStageId || 'Nieznany',
...
app.patch('/api/calls/:callId/report', async (req, res) => {
  ...
  // ═══ AUTO-CREATE: Jeśli nie ma kontaktu w GHL → stwórz go ═══
  const call = callsStore.find(c => c.callId === callId);
  const phone = call?.from || call?.to;
  if ((!contactId || contactId === 'unknown') && phone && (firstName || lastName)) {
    console.log(`[Report] No GHL contact for ${phone}. Creating...`);
    // Najpierw spróbuj znaleźć (może kontakt powstał w międzyczasie)
    ...
    if (ghlContact) {
      ...
      contactName = `${firstName || ''} ${lastName || ''}`.trim() || contactName;
      ...
      console.log(`[Report] ✅ GHL contact: ${ghlContact.id} for ${phone}`);
    }
  }
